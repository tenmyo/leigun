...
void close_cb(TcpHandle_t *handle, void *clientdata) {
  fprintf(stderr, "%s[%d] %s\n", __FILE__, __LINE__, __func__);
}

void write_cb(int status, TcpHandle_t *handle, void *clientdata) {
  fprintf(stderr, "%s[%d] %s %d\n", __FILE__, __LINE__, __func__, status);
  AsyncServer_Close(handle, &close_cb, NULL);
}

void read_cb(TcpHandle_t *handle, const void *buf, signed int len, void *clientdata) {
  if (len > 0) {
...
void accept_cb(int status, TcpHandle_t *handle, const char *host, int port, void *arg) {
  fprintf(stderr, "%s[%d] %s %d\n", __FILE__, __LINE__, __func__, status);
