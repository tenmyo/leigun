...
find . -name softgun -prune -o \( -name \*.c -or -name \*.cpp -or -name \*.cc \) |xargs -n1 -P4 $(dirname $0)/cleanup-tidy.sh
...
find . -name softgun -prune -o \( -name \*.c -or -name \*.cpp -or -name \*.cc -or -name \*.h \) |xargs -n12 -P4 $(dirname $0)/cleanup-format.sh -i
