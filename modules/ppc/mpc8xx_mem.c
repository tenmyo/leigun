...
#define MPC_MEMCO_MAGIC 0x64826459
typedef struct InternalDev {
	BusDevice *bdev;
	struct InternalDev *next;
} InternalDev;
...
MPC8xx_RebuildMmap(MPC8xx_MemCo * memco)
{
	...
	for (i = 0; i < 8; i++) {
		bdev = memco->bdev[i];
		if (bdev) {
			fprintf(stderr, "Delete mapping for dev %d\n", i);
	...
	if (memco->is_bootmap) {
		...
		for (i = 0; i < 8; i++) {
			...
			fprintf(stderr, "BR%d, base %08x, size 0x%08x\n", i, base, mapsize);
		}
	}
}
...
MemCo_ImmrWrite(uint32_t value, int spr, void *cd)
{
	...
	if (value != memco->immr) {
		...
		for (idev = memco->idev; idev; idev = idev->next) {
			Mem_AreaDeleteMappings(idev->bdev);
		}
		memco->immr = value;
		MPC8xx_MemCoMap(memco);
		for (idev = memco->idev; idev; idev = idev->next) {
			Mem_AreaAddMapping(idev->bdev, memco->immr, 64 * 1024,
					   MEM_FLAG_WRITABLE | MEM_FLAG_READABLE);
		}
		if (memco->is_bootmap) {
			MPC8xx_RebuildMmap(memco);
...
void
MPC8xx_RegisterIntDev(MPC8xx_MemCo * memco, BusDevice * bdev)
{

	InternalDev *idev = sg_calloc(sizeof(InternalDev));
	idev->next = memco->idev;
	idev->bdev = bdev;
	memco->idev = idev;
	Mem_AreaAddMapping(idev->bdev, memco->immr, 64 * 1024,
			   MEM_FLAG_WRITABLE | MEM_FLAG_READABLE);
...
{
	MPC8xx_MemCo *memco = sg_new(MPC8xx_MemCo);
	uint32_t configWord;
	uint32_t rstconf;
	uint32_t isb;
	uint32_t bdis;
	/* from hardreset configuration word */
	if (Config_ReadUInt32(&rstconf, "cpu", "rstconf") < 0) {
		fprintf(stderr, "Can not read CPU MPC8xx rstconf pin\n");
		exit(1468);
	}
	if (rstconf) {
		configWord = 0;
		...
		if (Config_ReadUInt32(&configWord, "cpu", "configWord") < 0) {
